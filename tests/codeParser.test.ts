...
describe('parseShareCode', () => {
  it('parses `<bot>_<code>` (current separator)', () => {
    expect(parseShareCode('vaultbot_ABCDE23456')).toEqual({
      botUsername: 'vaultbot',
      code: 'ABCDE23456',
    });
  });

  it('parses `@<bot>_<code>` and strips the leading @', () => {
    expect(parseShareCode('@vaultbot_ABCDE23456')).toEqual({
      botUsername: 'vaultbot',
      code: 'ABCDE23456',
    });
  ...
  it('still parses the legacy `<bot>:<code>` separator', () => {
  ...
  it('parses a bot username that itself contains underscores', () => {
    expect(parseShareCode('my_share_bot_ABCDE23456')).toEqual({
  ...
  it('returns null when the bot username does not end in "bot"', () => {
    expect(parseShareCode('myservice_ABCDE23456')).toBeNull();
  });
  ...
  it('returns null when the bot username is too short', () => {
    ...
    expect(parseShareCode('mybot_ABCDE23456')).toBeNull();
  });
});
